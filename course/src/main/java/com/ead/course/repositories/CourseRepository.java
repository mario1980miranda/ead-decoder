...
public interface CourseRepository extends JpaRepository<CourseModel, UUID>, JpaSpecificationExecutor<CourseModel> {
    ...
    @Query(
            value =
                    "select " +
                            "case when count(tcu) > 0 THEN true ELSE false END " +
                            "FROM tb_courses_users tcu " +
                            "WHERE tcu.course_id = :courseId and tcu.user_id = :userId",
            nativeQuery = true
    )
    boolean existsByCourseAndUser(@Param(value = "courseId") UUID courseId, @Param(value = "userId") UUID userId);

    @Modifying
    @Query(value = "insert into tb_courses_users values (:courseId, :userId)", nativeQuery = true)
    void saveCourseUser(@Param(value = "courseId") UUID courseId, @Param(value = "userId") UUID userId);
